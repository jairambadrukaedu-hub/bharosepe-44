-- Fix Critical RLS Policy Vulnerabilities (working with existing app_role enum)
...
-- 5. Add missing RLS policies for user_roles table
...
BEGIN
    ...
    -- Add valid proposal amount constraint if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.check_constraints 
                   WHERE constraint_name = 'valid_proposal_amount' 
                   AND table_name = 'dispute_proposals') THEN
