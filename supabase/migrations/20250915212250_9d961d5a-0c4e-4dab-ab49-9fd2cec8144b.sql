-- First remove constraint, update data, then add new constraint
-- Drop the existing status check constraint first
ALTER TABLE contracts DROP CONSTRAINT IF EXISTS contracts_status_check;

-- Now update existing contracts to new status names
UPDATE contracts 
SET status = CASE 
